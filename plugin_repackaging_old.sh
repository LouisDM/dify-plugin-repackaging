...
repackage(){
	...
	# Download packages for ARM64 architecture
	pip download ${PIP_PLATFORM} -r requirements.txt -d ./wheels --index-url ${PIP_MIRROR_URL} --trusted-host mirrors.aliyun.com --pre --only-binary=:all: --platform=manylinux_2_17_aarch64 --platform=manylinux2014_aarch64 --platform=manylinux_2_28_aarch64 --platform=manylinux_2_39_aarch64
	if [[ $? -ne 0 ]]; then
